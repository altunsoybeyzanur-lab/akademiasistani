  ...
  function AuthGate() {
    ...
    const social = async (prov) => {
      ...
      try {
        ...
        if (provider === 'google') {
          const { error } = await window._supabase.auth.signInWithOAuth({ provider: 'google' });
          if (error) throw error;
        } else {
          setTimeout(() => { setBusy(false); setAuthed(true); setRoute('welcome'); toast(`${prov} ${tr ? 'ile giriş yapıldı' : 'sign-in'}`, 'checkCircle'); }, 900);
        }
      } catch (err) {
      ...
      E('div', { className: 'auth-right' },
        E('div', { className: 'auth-card', style: { animationDelay: '100ms' } },
          ...
          E('div', { className: 'auth-social' },
            E('button', { className: 'auth-social-btn', onClick: () => social('Google') },
              E(Icon, { name: 'globe', size: 16 }), 'Google'
            ),
            E('button', { className: 'auth-social-btn', onClick: () => social('ORCID') },
              E(Icon, { name: 'award', size: 16 }), 'ORCID'
            )
          ),
